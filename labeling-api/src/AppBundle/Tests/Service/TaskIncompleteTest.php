...
class TaskIncompleteTest extends Tests\KernelTestCase
{
    ...
    public function testSimpleXml()
    {
        ...
        $video               = $this->videoFacade->save(Tests\Helper\VideoBuilder::create()->build());
        $task                = $this->labelingTaskFacade->save(
            Tests\Helper\LabelingTaskBuilder::create($project, $video)
                ->withLabelStructure($labelStructure->getLabelStructure())
                ->build()
        );
        $labeledThing        = $this->labeledThingFacade->save(
            Tests\Helper\LabeledThingBuilder::create()->withTask($task)->build()
        );
    ...
    public function testRequirementsXml()
    {
        ...
        $taskConfiguration = $this->taskConfigurationFacade->save(
            Tests\Helper\TaskConfigurationRequirementsBuilder::create($xml, $this->defaultUser)->build()
        );
