...
class QueueMaintainer extends Base
{
    /**
     ...
     * @var array
     */
    private $garbageQueues = array('worker.garbage-collection');
    ...
    /**
     * The current state. Currently this is an intenger from 0-9.
    ...
    {
        $this->setName('annostation:workerpool:maintainer')
            ->setDescription('RabbitMQ Maintainer Script');
    }
    ...
    {
        $exchange = $this->AMQPPoolConfig->workerMainExchange;
    ...
    private function promptForMessageAction()
    {
        ...
        $action = $dialog->select(
            $this->output,
            'Now we have a message you have following options:',
            $options,
            0
        );
    ...
    private function registerConsumer()
    {
        ...
        $this->channel = $this->AMQPPoolConfig
            ->openConnection()
            ->channel();
