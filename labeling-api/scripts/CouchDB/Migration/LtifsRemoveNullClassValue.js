...
if (process.argv[2] === '-h' || process.argv[2] === '--help' || process.argv.length <= 2) {
    console.log('Usage: LtifsRemoveNullClassValue.js [adminUrl]');
    process.exit(1);
}
...
function processDbQueue() {
    ...
    db.list({include_docs: true}, function(err, body) {
        ...
        documents.forEach(function(rawDocument) {
            ...
            if (document.classes.includes(null)) {
                document.classes = document.classes.filter(function(value) {
                    return value !== null;
                });
                bulkUpdates.push(document);
                console.log('Queued ' + document._id + ' to repair');
            }
        });
