...
class Utils {
  static destroyAndPurgeDocument(nanoAdmin, db, documentId, revision, callback) {
    db.get(documentId, {revs_info: true}, (err, body) => {
      ...
      const revisions = body._revs_info.map(revInfo => {
        return revInfo.rev;
      }).reverse();
  ...
  static purgeCouchDbReplicationDocument(nanoAdmin, documentId) {
    ...
    db.get(documentId, {revs: true, open_revs: 'all'}, (err, results) => {
      ...
      const okResult = results.find(result => {
        return ('ok' in result);
      });
      ...
      if (okResult === undefined) {
        ...
        const revisions = okResult.ok._revisions.ids.map((hash, index) => {
          return '' + (okResult.ok._revisions.ids.length - index) + '-' + hash;
        });

        return Utils._purgeDocument(nanoAdmin, db, documentId, revisions, err => {
          if (err) {
            return err;
          }
        ...
        return Utils.destroyAndPurgeDocument(nanoAdmin, db, documentId, okResult.ok._rev, err => {
          if (err) {
            return err;
          }
