...
resource "openstack_compute_floatingip_associate_v2" "labeltool-audi" {
    floating_ip = "${openstack_compute_floatingip_v2.labeltool-audi.address}"
    instance_id = "${openstack_compute_instance_v2.labeltool-audi.id}"
}

resource "null_resource" "labeltool-audi-provision" {
    provisioner "file" {
        source = "puppet"
