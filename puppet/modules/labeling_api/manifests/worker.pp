class labeling_api::worker(
  ...
  if $labeling_api::params::pouchdb_feature_enabled {
    supervisord::program { 'annostation-couchdb-replication-labeling-api-to-task-databases':
      command => "node ${app_dir}/scripts/CouchDB/ReplicatorManager/ReplicationManager.js 'http://${labeling_api::params::couchdb_user_read_only}:${labeling_api::params::couchdb_password_read_only}@${labeling_api::params::couchdb_host}:${labeling_api::params::couchdb_port}/' '(taskdb-project-)([a-z0-9_-]+)(-task-)([a-z0-9_-]+)' 'labeling_api'",
      autostart => true,
      autorestart => true,
      user => $user,
      directory => $app_dir,
